...
const DashboardShippingMethodPage = () => {
  ...
  return (
    <>
      <Button onClick={() => push("shipping-method/add")}>add shipping method</Button>
      <div className="font-medium text-lg my-3">Shipping Method is Available</div>
        ...
        {data?.shippingMethod.map((sm, idx) => (
          <div key={idx} className="p-2 border-rounded flex flex-col gap-3">
            <div className="flex gap-2">
              <Button
                onClick={() => push(`/dashboard/shipping-method/delete/${sm._id}`)}
                variant={"destructive"}
                size={"xs"}>
                Delete
              </Button>
              <Button
                onClick={() => push(`/dashboard/shipping-method/update/${sm._id}`)}
                variant={"default"}
                size={"xs"}>
                Update
              </Button>
